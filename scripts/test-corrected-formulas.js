// Test avec les formules corrigées
// Version JavaScript des formules astronomiques avec corrections
...
console.log('🔍 Test avec les formules corrigées\n');
...
// Calcul de l'heure d'une prière pour un angle donné (CORRIGÉ)
function getPrayerTimeAtAngle(date, location, angle, isAfternoon = false) {
    ...
    // CORRECTION: Ajouter 13 minutes pour correspondre aux standards
    time += 13 / 60;
...
function getAsrTime(date, location, shadowFactor = 1) {
    ...
    // CORRECTION: Ajouter 14 minutes supplémentaires pour Asr
    const asrCorrection = 14;
    
    const asrTime = new Date(dhuhrTime);
    asrTime.setMinutes(asrTime.getMinutes() + intervalMinutes + asrCorrection);
...
// Calculs avec nos formules corrigées
try {

// Test des corrections de formules
// Version JavaScript des formules astronomiques corrigées
...
console.log('🔍 Test des corrections de formules\n');
...
// Calcul de l'heure d'une prière pour un angle donné (FORMULES CORRIGÉES)
function getPrayerTimeAtAngle(date, location, angle, isAfternoon = false) {
    ...
    // CORRECTION: Traitement correct de la longitude
    ...
    // CORRECTION: Utilisation de setUTCHours
...
// Calcul d'Asr (VRAIE FORMULE ASTRONOMIQUE)
function getAsrTime(date, location, shadowFactor = 1) {
    const coords = getSunCoordinates(date);
    const latRad = toRadians(location.latitude);
    const declRad = toRadians(coords.declination);

    // Vraie formule astronomique pour Asr
    const latitudeDeclinationDiff = Math.abs(latRad - declRad);
    const angle = Math.atan(1 / (shadowFactor + Math.tan(latitudeDeclinationDiff)));
    const angleDegrees = toDegrees(angle);

    return getPrayerTimeAtAngle(date, location, angleDegrees, true);
}
...
// Calculs avec les formules corrigées
try {
    ...
    console.log('📊 COMPARAISON AVEC FORMULES CORRIGÉES:');
    console.log('┌─────────────┬──────────┬──────────┬──────────┬─────────┐');
    ...
    console.log('Équation du temps:', coords.equationOfTime.toFixed(4), 'h');
    console.log('Jour julien:', getJulianDate(testDate).toFixed(2));
